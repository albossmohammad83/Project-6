...
public:
  ...
  bool seatVacant(int seatNo) const;
   ...
   int getCount() const;
      //Effect: determines the number of reserved seats on this plane
      //Postcondition:  the PlaneFlight object is unchanged
      //Returns:  the number of reserved seats on this plane
  
private:
	  int seats[SIZE];    //plane seats (0- empty, 1 - full)
	  int count;          //number of reservations made so far
};